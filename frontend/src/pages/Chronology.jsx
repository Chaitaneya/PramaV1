...
import { Eye, Ear, Wind, Activity, FileText, Link, ShieldCheck, UserPlus, AlertCircle } from 'lucide-react';
    ...
    <Shell>
      ...
      <div className="flex flex-col flex-1 w-full bg-cream overflow-hidden" style={{ fontFamily: "'Inter', sans-serif" }}>
        ...
        {meta.pendingReviewCount > 0 && (
          <div className="bg-amber-100 border-b border-amber-200 px-5 py-2 flex items-center justify-between shrink-0 animate-in slide-in-from-top duration-300">
            <p className="text-[12px] text-amber-800 font-bold flex items-center gap-2">
              <AlertCircle size={14} className="text-amber-600" />
              {meta.pendingReviewCount} new client {meta.pendingReviewCount === 1 ? 'contribution' : 'contributions'} awaiting your review.
            </p>
            <button 
              onClick={() => setIsReviewPanelOpen(true)}
              className="text-[11px] font-bold text-amber-900 bg-amber-200 hover:bg-amber-300 px-3 py-1 rounded-lg transition-colors border border-amber-300"
            >
              Review Now
            </button>
          </div>
        ...
        <div className="px-5 py-2.5 border-b border-border bg-card-bg flex items-center justify-between shrink-0 z-10">
          ...
          <div className="flex items-center gap-2">
            ...
            <button className="btn-ghost flex items-center gap-1.5" onClick={() => setIsCollabPanelOpen(true)} style={{ borderColor: (meta.hasNewActivity || meta.pendingReviewCount > 0) ? '#4F46E5' : '#e2e8f0', color: (meta.hasNewActivity || meta.pendingReviewCount > 0) ? '#4F46E5' : '#64748b' }}>
              <UserPlus size={14} /> {t('collaborate', 'Collab')}
              {(meta.hasNewActivity || meta.pendingReviewCount > 0) && <span className="w-1.5 h-1.5 rounded-full bg-indigo-600 animate-pulse" />}
            </button>
      ...
      <CollabPanel isOpen={isCollabPanelOpen} onClose={() => setIsCollabPanelOpen(false)} caseId={id} />
      <ReviewPanel isOpen={isReviewPanelOpen} onClose={() => setIsReviewPanelOpen(false)} caseId={id} onUpdate={() => { load(true); }} />
    </Shell>
  );
}
