...
### FORMAT:
{
  ...
  router.post('/generate', async (req, res) => {
    try {
      ...
      if (caseData.hasNewActivity) {
        await Case.updateOne({ caseId }, { $set: { hasNewActivity: false } });
      }
      ...
      const allDeposits = sessions.flatMap(s => (s.deposits || [])
        .filter(d => d.status === 'approved') // Only use approved deposits
        .map(d => ({
          _id: d._id,
          content: d.content,
          sensoryTag: d.sensoryTag,
      ...
      if (!force && serverCache.has(cacheKey)) {
        ...
        return res.json({
          ...
          meta: {
            ...
            pendingReviewCount: sessions.reduce((acc, s) => acc + (s.deposits?.filter(d => d.status === 'pending').length || 0), 0),
